...
let generateSequential = (start: int, end: int): result<array<pattern>, generatorError> => {
  if start > end {
    ...
    let patterns = Belt.Array.map(numbers, Int.toString)
    Ok(patterns)
  }
}
...
let applyMutations = (word: string, level: mutationLevel): array<pattern> => {
  let capitalize = (s: string): string => {
    ...
    | None => s
    | Some(first) =>
      String.toUpperCase(String.fromCharCode(first)) ++
      String.sliceToEnd(s, ~start=1)
    }
  }
  ...
  | Aggressive =>
    Belt.Array.concat(
      ...
      [
        // Reverse
        String.split(word, "")->Belt.Array.reverse->String.concatMany(""),
      ]
    )
  }
}
  ...
  | Charset(charset, minLen, maxLen) =>
    ...
    let rec sumPowers = (start, end, acc) =>
      if start > end {
        ...
        sumPowers(start + 1, end, acc + Int.toFloat(base)->Math.pow(~exp=Int.toFloat(start))->Float.toInt)
      }
...
let calculateStats = (patterns: array<pattern>): patternStats => {
  ...
  if total == 0 {
    ...
    let min = Belt.Array.reduce(lengths, Int32.max_int, (acc, len) => min(acc, len))
    let max = Belt.Array.reduce(lengths, 0, (acc, len) => max(acc, len))
